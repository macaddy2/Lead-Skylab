...
export default function Generator() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const { generateContent, generateHashtags } = await import('../../lib/gemini');
            ...
            if (productProfile) {
                ...
                try {
                    const tags = await generateHashtags(generatedContent, productProfile, platform, 3);
                    const tagNames = tags.map(t => t.replace('#', ''));
                    setHashtags(prev => [...new Set([...prev, ...tagNames])]);
                } catch (e) {
                    console.warn('Hashtag generation failed', e);
                }
            } else {
                const template = contentTemplates.find(t => t.id === selectedTemplate);
                const generatedContent = template?.example || `🚀 Check out our new product!\n\nWe're solving [problem] for [audience].\n\nKey benefits:\n• Benefit 1\n• Benefit 2\n• Benefit 3\n\nLearn more: [link]`;
                setContent(generatedContent);
                setTitle(generatedContent.slice(0, 50) + '...');
            }
        } catch (err) {
            console.error('AI Generation Failed:', err);
            // Fallback to the old simulation if gemini is unconfigured or fails
            const template = contentTemplates.find(t => t.id === selectedTemplate);
